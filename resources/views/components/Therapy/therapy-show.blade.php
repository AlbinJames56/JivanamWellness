                ...
                <div class="bg-white/10 backdrop-blur-sm px-4 py-3 rounded-2xl border border-white/6  ">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            ...
                            <h1 class="text-2xl md:text-3xl font-semibold leading-tight text-background">{{ $therapy->title }}</h1>
                    ...
                    <div class="flex items-center gap-4">
                        @if(isset($therapy->rating))
                            <div class="flex items-center gap-1">
                                @for ($i = 1; $i <= 5; $i++)
                                    <svg class="w-4 h-4 {{ $i <= $therapy->rating ? 'text-yellow-400' : 'text-gray-300' }}" viewBox="0 0 24 24" fill="currentColor"><path d="M12 .587l3.668 7.568L24 9.423l-6 5.854L19.335 24 12 19.771 4.665 24 6 15.277 0 9.423l8.332-1.268z" /></svg>
                                @endfor
                            ...
                            @foreach($relatedTestimonials as $t)
                                <blockquote class="p-4 rounded-xl bg-card border border-border">
                                    <div class="flex items-start gap-3">
                                        <div class="w-12 h-12 rounded-full bg-muted/10 flex items-center justify-center text-muted-foreground font-medium">
            ...
            <aside class="space-y-6">
                <div class="p-5 rounded-2xl bg-card border border-border shadow-sm">
                    ...
                    <ul class="mt-3 text-sm text-muted-foreground space-y-2">
                        ...
                        <li><strong>Availability:</strong> <span class="ml-2">{{ $therapy->available ? 'Available' : 'Contact us' }}</span></li>
                        <li><strong>Price:</strong> <span class="ml-2">{{ $therapy->price ? ($therapy->price_currency ?? 'INR') . ' ' . number_format($therapy->price, 2) : '—' }}</span></li>
                    </ul>

                    <div class="mt-4">
                        <a href="#booking" class="w-full btn-primary block text-center">Book a Consultation</a>
                        <a href="mailto:info@example.com" class="w-full btn-secondary block text-center mt-3">Request Info</a>
                    </div>
                </div>
