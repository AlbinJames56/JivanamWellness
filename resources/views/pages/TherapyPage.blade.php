...
@section('content')
@php
    // keep local fallback only when controller didn't pass $testimonials
    ...
    // keep faqs from controller or local fallback
    if (! isset($faqs)) {
        $faqs = [ /* ... */ ];
    }
@endphp
    ...
    @forelse ($testimonials as $t)
        @php
            // therapy name and rating (safe)
            $therapyName = optional($t->therapy)->title ?? ($t->therapy ?? ($t['therapy'] ?? ''));
            $rating = $t->rating ?? ($t['rating'] ?? 0);

            // avatar handling (supports full URL, leading slash, or storage path)
        ...
        <article class="testimonial-card bg-card rounded-2xl border border-border p-6 max-w-md flex-shrink-0">
            {{-- layout: avatar + content when avatar exists; otherwise content-only with initial --}}
            @if($hasAvatar)
                <div class="flex items-start gap-4">
                    <img src="{{ $avatarSrc }}" alt="{{ $t->name ?? 'Avatar' }}" loading="lazy"
                         class="w-14 h-14 rounded-full object-cover flex-shrink-0" />

                    <div class="flex-1">
                        <div class="flex items-center justify-between">
                            <div class="text-sm font-medium">{{ $t->name ?? ($t['name'] ?? '') }}</div>
                        ...
                        <div class="mt-2">
                            <div class="flex items-center gap-1 mb-2">
                                @for ($i = 0; $i < 5; $i++)
                                    <svg class="w-4 h-4 {{ $i < $rating ? 'text-yellow-400' : 'text-gray-300' }}" viewBox="0 0 24 24" fill="currentColor">
                                        <path d="M12 .587l3.668 7.568L24 9.423l-6 5.854L19.335 24 12 19.771 4.665 24 6 15.277 0 9.423l8.332-1.268z" />
                                    </svg>
                                @endfor
                            </div>

                            <blockquote class="italic text-foreground leading-relaxed">"{{ $t->text ?? ($t['text'] ?? '') }}"</blockquote>
            ...
            @else
                {{-- No avatar: show initial + full text layout --}}
                <div>
                    <div class="flex items-start gap-4">
                        <div class="w-14 h-14 rounded-full bg-muted/10 flex items-center justify-center text-muted-foreground font-medium flex-shrink-0">
    ...
    @empty
        <div class="text-muted-foreground">No testimonials yet.</div>
    @endforelse
</div>
